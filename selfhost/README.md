...
+ **LSP** (sem Rust, via `lexcheck`) + **`lex` unificado** (`lexcli`:
build/run/fmt/test/check) + arrow+captura (A) + `any` (B) + **`lex test`** (C — 12
suítes SEM Rust) + span-tracking (D) + **`lex check`** (E-slice) em lex: ~5100
